...
@MQConsumer(name = "KYPushAPIImpl",code = "KY")
@Slf4j
public class KYPushAPIImpl {

    @Autowired
    private SourceServiceImpl sourceService;

    @SourceQuery(targetId = "gameEndTime",targetMgo = KyMgoPo.class)
    public void getPushBet(Integer siteId, List<KyMgoPo> list) throws ServiceException {
        try{
            log.info("KY站点siteId={}，接收数据rows={}",siteId,list.size());
            sourceService.insertSource(list, KyChessGamePo.class);
        }catch (Exception e){
            throw new ServiceException(e);
        }
    }

    @AuditReport(procedureName = "createAuditAndReportForKY",targetDao = AuditTotalDao.class)
    public void auditReport(Integer siteId,String result) {
        log.info("审计报表Procedure返回结果result={}",result);
        if(result.isEmpty()){
            log.error("KY站点siteId={}，执行审计报表存储过程为空，请查看传参",siteId);
        }
    }
}
