...
<table id="table_grid"  style="min-width:1000px;" data-options="fitColumns:true,rownumbers:true,striped:true">
	...
	<thead>
		...
		<th data-options="field:'cargoBusinessAuthorId',align:'center'" >货物企业认证id</th>
		<th data-options="field:'userId',align:'center'">用户id</th>
		<th data-options="field:'companyName',align:'center'" width='10%'>企业名称</th>
		<th data-options="field:'legalPerson',align:'center'" width='10%'>法人姓名</th>
		<th data-options="field:'legalPhone',align:'center'" width='10%'>法人电话</th>
		<th data-options="field:'businessNo',align:'center'" width='10%'>企业登记号</th>
		<th data-options="field:'status',align:'center',formatter:CargoCompanyAuthorMgr.statusFormatter" width='10%'>状态</th>
		...
		<th data-options="field:'action',align:'center',formatter:CargoCompanyAuthorMgr.actionFormatter" width='25%'>操作 </th>							
		</tr>
	</thead>
 	<tbody>
 	</tbody>
</table>
<div id="acceptCargoCompanyAuthorDialog"></div>
<div id="refuseCargoCompanyAuthorDialog"></div>
<div id="detailCargoCompanyAuthorDialog"></div>
 <script>	 
 eason.register("CargoCompanyAuthorMgr");
$(function(){ 
	$('#table_grid').datagrid({ 
		width:"auto",
	    height:"auto", 
	    url:"<%=path%>/admin/author/cargocompany/query",
		checkOnSelect:false,
	  	selectOnCheck:false,
	  	singleSelect:true,
		striped:true, 
...
CargoCompanyAuthorMgr.statusFormatter = function(value, row, index) {
	var info;
	switch(value){
...
CargoCompanyAuthorMgr.actionFormatter = function(value, row, index) {
	var detail = "<a href='#' class='easyui-linkbutton' data-options=\"iconCls:'icon-search',plain:true\" onclick=\"CargoCompanyAuthorMgr.detail('"+ row.cargoBusinessAuthorId + "',event)\">详情</a>";
	var accept = "<a href='#' class='easyui-linkbutton' data-options=\"iconCls:'icon-search',plain:true\" onclick=\"CargoCompanyAuthorMgr.accept('"+ row.cargoBusinessAuthorId + "',event)\">通过认证</a>";
	var refuse = "<a href='#' class='easyui-linkbutton' data-options=\"iconCls:'icon-search',plain:true\" onclick=\"CargoCompanyAuthorMgr.refuse('"+ row.cargoBusinessAuthorId + "',event)\">拒绝认证</a>";
	if(row.status==1){
...
CargoCompanyAuthorMgr.detail=function(cargoBusinessAuthorId,e){
	var dialogType="detailCargoCompanyAuthorDialog";
	var info="查看详情";
	var params={};
	CargoCompanyAuthorMgr.cargoBusinessAuthorId=cargoBusinessAuthorId;//绑定全局变量：此id是method的id
	$.post("<%=path%>/admin/author/cargocompany/detail",{"cargoBusinessAuthorId":cargoBusinessAuthorId},
			function(data){
				var vo=data.returnObj;
				CargoCompanyAuthorMgr.vo=vo;
				params=vo;
				params.width=820;
				params.height=600;
				openDiog3(dialogType,info,"dialog/"+dialogType,"form_"+dialogType,params,function(){
					...
					CargoCompanyAuthorMgr.initDetail();//给详细页面设置参数值
				});
			}
	);
};

CargoCompanyAuthorMgr.initDetail=function(){
	var vo = CargoCompanyAuthorMgr.vo;
...
CargoCompanyAuthorMgr.accept=function(cargoBusinessAuthorId,e){
	var dialogType="acceptCargoCompanyAuthorDialog";
	var info="接受认证";
	var params={};
	params.width=510;
	params.height=330;
	params.cargoBusinessAuthorId=cargoBusinessAuthorId;
...
CargoCompanyAuthorMgr.refuse=function(cargoBusinessAuthorId,e){
	var dialogType="refuseCargoCompanyAuthorDialog";
	var info="拒绝认证";
	var params={};
	params.width=510;
	params.height=330;
