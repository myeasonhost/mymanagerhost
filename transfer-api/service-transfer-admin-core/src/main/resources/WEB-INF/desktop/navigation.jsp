...
#ayscNavigation h3{
	...
	font-weight:normal;
	cursor:pointer;
	background:url("<%=request.getContextPath()%>/image/admin/HT-nav-normalbg.png") repeat-x ;
}
</style>
	...
	$.post("<%=request.getContextPath()%>/admin/user/getResourcesOfUser",
		function(data){
			...
			$("#ayscNavigation h3").bind("mouseover",function(){
		 		$(this).css("background","url('<%=request.getContextPath()%>/image/admin/HT-nav-hoverbg.png')");
		 	}).bind("mouseout",function(){
		 		$(this).css("background","url('<%=request.getContextPath()%>/image/admin/HT-nav-normalbg.png')");
		 	});
		},
		'json'
	);
...
function addTabToContentRegion(id,title,href){
	...
	if(isExists){
		...
		var content="<iframe src = <%=request.getContextPath()%>"+href + " width='100%' height='100%' scrolling='auto' frameborder='0'/>";
