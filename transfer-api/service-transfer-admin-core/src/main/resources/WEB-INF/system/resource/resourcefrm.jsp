...
<html>
   ...
   //资源的查看
	$(function(){
		...
		//监听事件 --> 单击节点 --> 初始化时为节点添加单击事件( 展开节点 --> 为节点添加展开监听事件,收缩节点 --> 为节点添加收缩监听事件)
		$('#resourceTree').tree({
			url:'<%=request.getContextPath()%>/admin/resource/getResources',
		...
		function ResourceAction(){
			...
			this.resourceSaveOrUpdate= function(){
				...
				$('#resourceForm').form('submit',{
					url:'<%=request.getContextPath()%>/admin/resource/saveResource',
					dataType: 'json',
				    onSubmit:function(){
